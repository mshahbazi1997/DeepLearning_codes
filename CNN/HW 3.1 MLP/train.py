...
def train(x_train, y_train, x_val, y_val, x_test, y_test, model, sess, log_file):
    ...
    train_writer = tf.summary.FileWriter(train_log_file, graph=sess.graph)
    val_writer = tf.summary.FileWriter(val_log_file, graph=sess.graph)
    test_writer = tf.summary.FileWriter(test_log_file, graph=sess.graph)
    ...
    for epoch in range(FLAGS.num_epoch):
        for batch in range(num_batch):
            ...
            feed_dict = {model.placeholders['batch_images']: x_train[batch_range],
        ...
        feed_dict = {model.placeholders['batch_images']: x_train,
        ...
        feed_dict = {model.placeholders['batch_images']:x_val,
                    model.placeholders['batch_labels']:y_val}
        
        val_feed_dict.update(feed_dict)
        ...
        feed_dict = {model.placeholders['batch_images']:x_test, 
