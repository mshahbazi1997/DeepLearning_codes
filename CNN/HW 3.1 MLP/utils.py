...
def glorot_initializer(shape, name=None):
    ...
    limit = tf.dtypes.cast(limit, 'float32', name=None)

    
    W = tf.Variable(tf.random.uniform(shape, minval = -limit, maxval = limit), name = name)
...
def normal_initializer(shape, stddev, name=None):
    ...
    W = tf.Variable(tf.truncated_normal(shape, mean= 0, stddev= stddev), name= name)
